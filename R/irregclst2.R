#' @title  Convert Irregular Longitudinal Data to Regular Intervals and Perform Clustering using inear regression model for replacing repeated responses (LRRS) method
...
#'
#' @details
#' The \code{irregclst2} function handles irregular longitudinal data by:
#' \itemize{
#'  \item Interpolating response values at regular time intervals and replaing the last responses using linear regression model.
...
irregclst2 <- function(data, subject_id_col, time_col, response_col, rel, interval_length) {
  #library(ggplot2)
  suppressWarnings({
  ...
  # Impute NA values using linear regression for each subject
  subject_ids <- unique(regular_data$subject_id)

  for (i in subject_ids) {
    # Extract data for the current subject
    subject_data <- regular_data[regular_data$subject_id == i, ]

    # Check if there are NA values in the response
    if (any(is.na(subject_data$response))) {
      # Separate non-NA and NA data
      non_na_data <- subject_data[!is.na(subject_data$response), ]
      na_data <- subject_data[is.na(subject_data$response), ]

      # Ensure there are at least two non-NA data points for fitting a regression model
      if (nrow(non_na_data) >= 2) {
        # Fit linear regression model: response ~ time
        n <- nrow(non_na_data)
        x <- non_na_data$time
        y <- non_na_data$response
        x_mean <- sum(x) / n
        y_mean <- sum(y) / n
        ...
        # Predict NA values
        predicted_values <- b0 + b1 * na_data$time

        # Fill in the NA values
        na_indices <- which(regular_data$subject_id == i & is.na(regular_data$response))
        regular_data$response[na_indices] <- predicted_values
      }
    }
  }
